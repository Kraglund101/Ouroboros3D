...
# ──────────────────────────────  Constants  ──────────────────────────────── #
noise_mode = "independent"  # 'independent' | 'shared' | 'interpolated'
...
if __name__ == "__main__":
    ...
    mv_model = mv_model(pipeline.unet)
    recon_model = hydra.utils.instantiate(conf.recon_model)


    state_dict = torch.load(mv_model_ckpt, map_location="cpu")

    # strip the 'mv_model.' prefix if present
    clean_state_dict = {
        k.replace("mv_model.", "", 1): v
        for k, v in state_dict.items()
    }

    missing, unexpected = mv_model.load_state_dict(clean_state_dict, strict=False)
    print(f"✅ loaded MVModel – {len(missing)} missing / {len(unexpected)} unexpected")
